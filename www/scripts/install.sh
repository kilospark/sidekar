...
# Get latest version from sidekar.dev API
if [ -z "$VERSION" ]; then
  VERSION="$(curl -fsSL "https://${DOMAIN}/v1/version?current=0.0.0" | grep '"latest"' | cut -d'"' -f4)"
  if [ -n "$VERSION" ]; then
    VERSION="v${VERSION}"
  fi
fi
...
for dir in /usr/local/bin "$HOME/.local/bin"; do
  for old_bin in sidekar-mcp sidekar-rs webact webact-mcp webact-rs; do
