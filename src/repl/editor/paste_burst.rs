//! Paste-burst detection for terminals that don't wrap pastes in bracketed-paste.
//!
//! Apple Terminal, Windows consoles, and some other terminals deliver drag-and-drop
//! and clipboard pastes as a rapid sequence of typed characters. Without detection,
//! a dropped image path arrives as keystrokes and is never recognized as a paste.
//!
//! `PasteBurst` is a pure state machine. The caller feeds plain char events and
//! applies the returned decisions: buffer / retain / retro-grab. On an idle tick
//! `flush_if_due` returns the accumulated burst as a single paste string.
//!
//! Ported from codex-rs `codex-rs/tui/src/bottom_pane/paste_burst.rs`.
...
impl PasteBurst {
    ...
    pub(super) fn on_plain_char(&mut self, ch: char, now: Instant) -> CharDecision {
    ...
    pub(super) fn on_plain_char_no_hold(&mut self, now: Instant) -> Option<CharDecision> {
        self.note_plain_char(now);
    ...
    /// Test-only: unconditionally flush as if the idle timeout had elapsed
    /// and the enter-suppression window had expired.
    #[cfg(test)]
    pub(super) fn force_flush(&mut self) -> FlushResult {
        let result = if self.is_active_internal() {
            self.active = false;
            FlushResult::Paste(std::mem::take(&mut self.buffer))
        } else if let Some((ch, _)) = self.pending_first_char.take() {
    ...
    pub(super) fn append_newline_if_active(&mut self, now: Instant) -> bool {
        if self.is_active() {
            self.buffer.push('\n');
            self.burst_window_until = Some(now + PASTE_ENTER_SUPPRESS_WINDOW);
            true
    ...
    pub(super) fn newline_should_insert_instead_of_submit(&self, now: Instant) -> bool {
        let in_burst_window = self.burst_window_until.is_some_and(|until| now <= until);
        self.is_active() || in_burst_window
    }

    pub(super) fn begin_with_retro_grabbed(&mut self, grabbed: String, now: Instant) {
        if !grabbed.is_empty() {
    ...
    pub(super) fn append_char_to_buffer(&mut self, ch: char, now: Instant) {
        self.buffer.push(ch);
        self.burst_window_until = Some(now + PASTE_ENTER_SUPPRESS_WINDOW);
    }

    /// Retro-capture heuristic: only grab if the prefix has whitespace or is long
    /// (>= 16 chars). Short word-like prefixes stay as normal typing.
    pub(super) fn decide_begin_buffer(
        &mut self,
        now: Instant,
        before: &str,
        retro_chars: usize,
    ...
    pub(super) fn flush_before_modified_input(&mut self) -> Option<String> {
        if !self.is_active() {
            return None;
        }
        self.active = false;
    ...
    pub(super) fn clear_window_after_non_char(&mut self) {
        self.consecutive_plain_char_burst = 0;
        self.last_plain_char_time = None;
        self.burst_window_until = None;
        self.active = false;
...
pub(super) fn retro_start_index(before: &str, retro_chars: usize) -> usize {
    if retro_chars == 0 {
        return before.len();
    }
    before
...
mod tests {
    ...
    #[test]
    fn two_fast_chars_start_buffer_and_flush_as_paste() {
        let mut burst = PasteBurst::default();
        let t0 = Instant::now();
        assert!(matches!(
            burst.on_plain_char('a', t0),
    ...
    #[test]
    fn long_path_burst_flushes_as_paste() {
        let mut burst = PasteBurst::default();
        let path = "/Users/alice/Desktop/screenshot.png";
        let mut now = Instant::now();

        for (i, ch) in path.chars().enumerate() {
            now += Duration::from_millis(1);
            let decision = burst.on_plain_char(ch, now);
            match decision {
                CharDecision::RetainFirstChar => assert_eq!(i, 0),
                CharDecision::BeginBufferFromPending => {
                    assert_eq!(i, 1);
                    burst.append_char_to_buffer(ch, now);
                }
