...
fn is_smart_wait_command(tool: &str) -> bool {
    matches!(
        tool,
        "navigate" | "click" | "fill" | "select" | "type" | "clipboard" | "inserttext"
    )
}
...
fn build_action_args(tool: &str, action: &Value) -> Vec<String> {
    ...
    match tool {
        ...
        "click" | "hover" => {
            // Support mode param in batch actions
            if let Some(mode) = action.get("mode").and_then(Value::as_str) {
        ...
        "keyboard" | "paste" | "inserttext" => {
            if let Some(text) = action.get("text").and_then(Value::as_str) {
                args.push(text.to_string());
            }
        }
        "clipboard" => {
            if let Some(html) = action.get("html").and_then(Value::as_str) {
