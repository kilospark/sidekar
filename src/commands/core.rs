...
pub(super) async fn cmd_click(ctx: &mut AppContext, selector: &str) -> Result<()> {
    ...
    // Enable network monitoring to detect AJAX triggered by the click
    ...
    // Check if any network requests fired during the 150ms wait.
    // If so, wait for network quiet before generating the brief.
    let mut inflight: i32 = 0;
    let mut had_network = false;

    // Drain any events that arrived during the 150ms sleep
    loop {
    ...
    // If network activity was detected, wait for it to settle (up to 3s)
    if had_network && inflight > 0 {
        ...
        while Instant::now() < net_deadline {
            ...
            let Some(event) = cdp.next_event(remain).await? else {
                // No event within timeout — check if we've been quiet long enough
        ...
        // Wait for a render frame so the DOM reflects the network response
...
pub(super) async fn cmd_new_tab(ctx: &mut AppContext, url: Option<&str>) -> Result<()> {
    let mut state = ctx.load_session_state()?;
    // Prune stale tab IDs (tabs closed outside sidekar) before checking limit.
    // Only prune when we can actually list tabs — don't discard state on failure.
...
pub(super) async fn cmd_uninstall(_ctx: &mut AppContext) -> Result<()> {
    ...
    // Clean up config directory (~/.config/sidekar/)
    ...
    // Clean up old agent skills remnant
