...
/// Add sidekar to all detected MCP clients.
pub fn configure_clients() {
    ...
    if has_binary("codex") {
        ...
        configure_cli_client_grep(
            ...
            &["codex", "mcp", "list"],
            "sidekar",
            &["codex", "mcp", "add", "sidekar", "--", &binary_path, "mcp"],
        );
    }
    ...
    println!();
    if any {
        println!("Done! Restart your MCP client to start using sidekar.");
    } else {
        println!("  No MCP clients detected. Add manually to your client config:");
        println!();
        println!(
            "  {{ \"mcpServers\": {{ \"sidekar\": {{ \"command\": \"{binary_path}\", \"args\": [\"mcp\"] }} }} }}"
        );
    }
}

/// Remove sidekar from all detected MCP clients.
pub fn remove_clients() {
    println!();
    println!("Removing sidekar from MCP clients...");
    ...
    if has_binary("claude") {
        let exists = run_silent(&["claude", "mcp", "get", "sidekar"]);
        if exists {
            any = true;
    ...
    if has_binary("codex") {
        if run_grep(&["codex", "mcp", "list"], "sidekar") {
            any = true;
            if run_silent(&["codex", "mcp", "remove", "sidekar"]) {
                println!("  Codex: removed");
            } else {
                println!("  Codex: failed to remove (try: codex mcp remove sidekar)");
            }
        }
    }

    if has_binary("gemini") {
        if run_grep(&["gemini", "mcp", "list"], "sidekar") {
            any = true;
            if run_silent(&["gemini", "mcp", "remove", "-s", "user", "sidekar"]) {
                println!("  Gemini CLI: removed");
            } else {
                println!("  Gemini CLI: failed to remove (try: gemini mcp remove -s user sidekar)");
            }
        }
    }
    ...
    // -- Clean up data created by sidekar --
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        for subdir in &["skills/sidekar", "plugins/cache/sidekar"] {
    ...
    if any {
        ...
        println!("  Nothing to uninstall — no sidekar configs or data found.");
    }
}
...
fn upsert_mcp_servers(path: &Path, binary_path: &str, create_if_missing: bool) -> Option<String> {
    ...
    if let Some(existing) = servers_map.get("sidekar") {
...
/// Remove sidekar from an mcpServers config file.
/// Returns `None` if file doesn't exist or has no sidekar entry.
...
fn upsert_opencode(path: &Path, binary_path: &str) -> Option<String> {
    ...
    if mcp_map.contains_key("sidekar") {
        return Some("already configured".into());
    }
